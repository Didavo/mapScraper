"""
Scraper für die Stadt Creglingen.
API: https://api.cross-7.de/public/calendar/1860/events

Besonderheit: Eigene JSON-API (Cross-7 CMS)
...
class CrelingenScraper(BaseScraper):
    """Scraper für Creglingen Veranstaltungen (Cross-7 Calendar API)."""

    SOURCE_NAME = "Stadt Creglingen"
    BASE_URL = "https://www.creglingen.de"
    EVENTS_URL = "https://www.creglingen.de/veranstaltungen"

    # API-Endpunkt
    API_URL = "https://api.cross-7.de/public/calendar/1860/events"
    PAGE_SIZE = 200

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "97993 Creglingen"
    ...
    def _parse_api_event(self, item: Dict[str, Any]) -> Optional[ScrapedEvent]:
        ...
        url = f"{self.BASE_URL}/de/tourismus-freizeit/veranstaltungen-vereine/veranstaltungskalender?c7-item={target_id}" if target_id else self.EVENTS_URL
        ...
        if target_id:
            external_id = f"creglingen_{target_id}_{event_date}"
        else:
            import hashlib
            hash_input = f"{name}_{event_date}".encode("utf-8")
            hash_id = hashlib.md5(hash_input).hexdigest()[:8]
            external_id = f"creglingen_{hash_id}_{event_date}"
    ...
    def _parse_time(self, time_str: Optional[str]) -> Optional[time_class]:
        """Parst Zeitformat: '15:15:00' -> time(15, 15)"""
        if not time_str:
            return None
