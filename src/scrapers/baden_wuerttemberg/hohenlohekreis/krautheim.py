"""
Scraper für die Stadt Krautheim.
Website: https://www.krautheim.de/freizeit-erlebnis/veranstaltungen/veranstaltungskalender
Pagination: /seite-X/
"""
...
class KrautheimScraper(BaseScraper):
    """Scraper für Krautheim Veranstaltungen mit Pagination."""

    SOURCE_NAME = "Stadt Krautheim"
    BASE_URL = "https://www.krautheim.de"
    EVENTS_URL = "https://www.krautheim.de/freizeit-erlebnis/veranstaltungen/veranstaltungskalender/seite-1/suche-none"

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74238 Krautheim"

    # CSS-Selektoren für Krautheim
    SELECTORS = {
    ...
    def parse_german_date(self, date_str: str) -> Optional[date_class]:
        """
        Parst deutsches Datumsformat: "27.02.2026"
        """
        if not date_str:
            return None
    ...
    def parse_time(self, time_str: str) -> Optional[time_class]:
        """
        Parst Uhrzeitformat: "20:00 Uhr bis 22:00 Uhr" -> nimmt Startzeit
        """
        if not time_str:
            return None
    ...
    def get_all_page_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        Pagination-Struktur bei Krautheim:
        ...
        # Generiere alle URLs
        base_pattern = "https://www.krautheim.de/freizeit-erlebnis/veranstaltungen/veranstaltungskalender/seite-{}/suche-none"
    ...
    def _generate_external_id(
        ...
        # Methode 1: ID aus Container-Attribut (id="hwveranstaltung__record__235")
        container_id = container.get("id", "")
        match = re.search(r"__(\d+)$", container_id)
        if match:
            return f"{match.group(1)}_{event_date}"

        # Methode 2: ID aus URL extrahieren (/veranstaltungskalender/235/...)
        if url:
        ...
        return f"krautheim_{hash_id}_{event_date}"
