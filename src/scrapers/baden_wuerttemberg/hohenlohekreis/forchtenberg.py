"""
Scraper für die Stadt Forchtenberg.
Website: https://www.forchtenberg.de/freizeit-tourismus/veranstaltungskalender
Pagination: /seite-X/
"""
...
class ForchtenbergScraper(BaseScraper):
    """Scraper für Forchtenberg Veranstaltungen mit Pagination."""

    SOURCE_NAME = "Stadt Forchtenberg"
    BASE_URL = "https://www.forchtenberg.de"
    EVENTS_URL = "https://www.forchtenberg.de/freizeit-tourismus/veranstaltungskalender/seite-1/suche-none"

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74670 Forchtenberg"

    # CSS-Selektoren für Forchtenberg
    SELECTORS = {
    ...
    def parse_german_date(self, date_str: str) -> Optional[date_class]:
        """
        Parst deutsches Datumsformat: "06.03.2026"
        """
        if not date_str:
            return None
    ...
    def parse_time(self, time_str: str) -> Optional[time_class]:
        """
        Parst Uhrzeitformat: "09:00 Uhr bis 17:00 Uhr" -> nimmt Startzeit
        """
        if not time_str:
            return None
    ...
    def get_all_page_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        Pagination-Struktur:
        - Links mit href="/freizeit-tourismus/veranstaltungskalender/seite-X/suche-none"
        - "Letzte Seite" Link mit title="Letzte Seite"
        """
        max_page = 1

        # Methode 1: Finde "Letzte Seite" Link
        last_page_link = soup.select_one('a[title="Letzte Seite"]')
        if last_page_link:
        ...
        if max_page == 1:
            pagination_links = soup.select('.hw_pagination a[href*="/seite-"]')
        ...
        # Generiere alle URLs
        base_pattern = "https://www.forchtenberg.de/freizeit-tourismus/veranstaltungskalender/seite-{}/suche-none"
    ...
    def _generate_external_id(
        ...
        # Methode 1: ID aus Container-Attribut (id="hwveranstaltung__record__535")
        container_id = container.get("id", "")
        match = re.search(r"__(\d+)$", container_id)
        if match:
            return f"{match.group(1)}_{event_date}"

        # Methode 2: ID aus URL extrahieren (/veranstaltungskalender/535/...)
        if url:
        ...
        return f"forchtenberg_{hash_id}_{event_date}"
