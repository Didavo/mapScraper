"""
Scraper für die Stadt Öhringen.
API: https://api.cross-7.de/public/calendar/333/events
...
class OehringenScraper(BaseScraper):
    """Scraper für Öhringen Veranstaltungen (Cross-7 Calendar API)."""

    SOURCE_NAME = "Stadt Öhringen"
    BASE_URL = "https://www.oehringen.de"
    EVENTS_URL = "https://www.oehringen.de/freizeit-tourismus/veranstaltungskalender"

    # API-Endpunkt
    API_URL = "https://api.cross-7.de/public/calendar/333/events"
    PAGE_SIZE = 100

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74613 Öhringen"
    ...
    def _parse_api_event(self, item: Dict[str, Any]) -> Optional[ScrapedEvent]:
        ...
        if target_id:
            external_id = f"oehringen_{target_id}_{event_date}"
        else:
            import hashlib
            hash_input = f"{name}_{event_date}".encode("utf-8")
            hash_id = hashlib.md5(hash_input).hexdigest()[:8]
            external_id = f"oehringen_{hash_id}_{event_date}"
