"""
Scraper für die Gemeinde Michelfeld.
API: https://api.cross-7.de/public/calendar/1786/events
...
class MichelfeldScraper(BaseScraper):
    """Scraper für Michelfeld Veranstaltungen (Cross-7 Calendar API)."""

    SOURCE_NAME = "Gemeinde Michelfeld"
    BASE_URL = "https://www.michelfeld.de"
    EVENTS_URL = "https://www.michelfeld.de/leben-wohnen/veranstaltungskalender"

    # API-Endpunkt
    API_URL = "https://api.cross-7.de/public/calendar/1786/events"
    PAGE_SIZE = 200

    # Kategorie-IDs die ausgeschlossen werden sollen
    EXCLUDE_CATEGORY_IDS = [186403, 186400, 186401]

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74545 Michelfeld"
    ...
    def _parse_api_event(self, item: Dict[str, Any]) -> Optional[ScrapedEvent]:
        ...
        if target_id:
            external_id = f"michelfeld_{target_id}_{event_date}"
        else:
            import hashlib
            hash_input = f"{name}_{event_date}".encode("utf-8")
            hash_id = hashlib.md5(hash_input).hexdigest()[:8]
            external_id = f"michelfeld_{hash_id}_{event_date}"
