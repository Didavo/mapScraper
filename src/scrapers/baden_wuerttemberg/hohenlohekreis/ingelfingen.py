"""
Scraper für die Stadt Ingelfingen.
Website: https://www.ingelfingen.de/de/freizeit-kultur/veranstaltungen
Pagination: /seite-X/
"""
...
class IngelfingenScraper(BaseScraper):
    """Scraper für Ingelfingen Veranstaltungen mit Pagination."""

    SOURCE_NAME = "Stadt Ingelfingen"
    BASE_URL = "https://www.ingelfingen.de"
    EVENTS_URL = "https://www.ingelfingen.de/de/freizeit-kultur/veranstaltungen/seite-1/suche-none"

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74653 Ingelfingen"

    # CSS-Selektoren für Ingelfingen
    SELECTORS = {
    ...
    def parse_german_date(self, date_str: str) -> Optional[date_class]:
        """
        Parst deutsches Datumsformat: "28.02.2026"
        """
        if not date_str:
            return None
    ...
    def parse_time(self, time_str: str) -> Optional[time_class]:
        """
        Parst Uhrzeitformat: "09:00 Uhr" oder "09:00"
        """
        if not time_str:
            return None
    ...
    def get_all_page_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        URL-Muster: /seite-1/, /seite-2/, etc.
        """
        max_page = 1

        # Suche nach Pagination-Links mit /seite-X/
        ...
        # Alternativ: Suche nach "Letzte Seite" Link
        last_page_link = soup.select_one('a[title*="letzte"], a[aria-label*="letzte"], a.last')
        if last_page_link:
        ...
        # Generiere alle URLs
        base_pattern = "https://www.ingelfingen.de/de/freizeit-kultur/veranstaltungen/seite-{}/suche-none"
    ...
    def _generate_external_id(
        ...
        # Methode 1: ID aus Container-Attribut (id="hwveranstaltung__record__1107")
        container_id = container.get("id", "")
        match = re.search(r"__(\d+)$", container_id)
        if match:
            return f"{match.group(1)}_{event_date}"

        # Methode 2: ID aus URL extrahieren (/veranstaltungen/1107/...)
        if url:
        ...
        return f"ingelfingen_{hash_id}_{event_date}"
