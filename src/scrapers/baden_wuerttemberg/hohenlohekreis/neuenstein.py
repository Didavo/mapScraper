"""
Scraper für die Stadt Neuenstein.
Website: https://www.neuenstein.de/freizeit-tourismus/veranstaltungskalender
Pagination: /seite-X/
"""
...
class NeuensteinScraper(BaseScraper):
    """Scraper für Neuenstein Veranstaltungen mit Pagination."""

    SOURCE_NAME = "Stadt Neuenstein"
    BASE_URL = "https://www.neuenstein.de"
    EVENTS_URL = "https://www.neuenstein.de/freizeit-tourismus/veranstaltungskalender/seite-1/suche-none"

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74632 Neuenstein"

    # CSS-Selektoren für Neuenstein
    SELECTORS = {
    ...
    def parse_german_date(self, date_str: str) -> Optional[date_class]:
        """
        Parst deutsches Datumsformat: "08.02.2026"
        """
        if not date_str:
            return None
    ...
    def parse_time(self, time_str: str) -> Optional[time_class]:
        """
        Parst Uhrzeitformat: "14:30 Uhr bis 17:00 Uhr" -> nimmt Startzeit
        """
        if not time_str:
            return None
    ...
    def get_all_page_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        Pagination-Struktur bei Neuenstein:
        - Links mit title="Zur Seite X"
        - Optional: "Letzte Seite" Link
        """
        max_page = 1

        # Methode 1: Finde "Letzte Seite" Link (falls vorhanden)
        ...
        # Generiere alle URLs
        base_pattern = "https://www.neuenstein.de/freizeit-tourismus/veranstaltungskalender/seite-{}/suche-none"
    ...
    def _generate_external_id(
        ...
        # Methode 1: ID aus Container-Attribut (id="hwveranstaltung__record__631")
        container_id = container.get("id", "")
        match = re.search(r"__(\d+)$", container_id)
        if match:
            return f"{match.group(1)}_{event_date}"

        # Methode 2: ID aus URL extrahieren (/veranstaltungskalender/631/...)
        if url:
        ...
        return f"neuenstein_{hash_id}_{event_date}"
