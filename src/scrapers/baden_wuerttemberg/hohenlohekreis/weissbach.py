"""
Scraper für die Gemeinde Weißbach.
Website: https://www.gemeinde-weissbach.de/freizeit-erholung/veranstaltungskalender
Pagination: /seite-X/
"""
...
class WeissbachScraper(BaseScraper):
    """Scraper für Weißbach Veranstaltungen mit Pagination."""

    SOURCE_NAME = "Gemeinde Weißbach"
    BASE_URL = "https://www.gemeinde-weissbach.de"
    EVENTS_URL = "https://www.gemeinde-weissbach.de/freizeit-erholung/veranstaltungskalender/seite-1/suche-none"

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74679 Weißbach"

    # CSS-Selektoren für Weißbach
    SELECTORS = {
    ...
    def parse_german_date(self, date_str: str) -> Optional[date_class]:
        """
        Parst deutsches Datumsformat: "10.02.2026"
        """
        if not date_str:
            return None
    ...
    def parse_time(self, time_str: str) -> Optional[time_class]:
        """
        Parst Uhrzeitformat: "19:30 Uhr" -> 19:30
        """
        if not time_str:
            return None
    ...
    def get_all_page_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        Pagination-Struktur bei Weißbach:
        ...
        # Generiere alle URLs
        base_pattern = "https://www.gemeinde-weissbach.de/freizeit-erholung/veranstaltungskalender/seite-{}/suche-none"
    ...
    def _generate_external_id(
        ...
        # Methode 1: ID aus Container-Attribut (id="hwveranstaltung__record__296")
        container_id = container.get("id", "")
        match = re.search(r"__(\d+)$", container_id)
        if match:
            return f"{match.group(1)}_{event_date}"

        # Methode 2: ID aus URL extrahieren (/veranstaltungskalender/296/...)
        if url:
        ...
        return f"weissbach_{hash_id}_{event_date}"
