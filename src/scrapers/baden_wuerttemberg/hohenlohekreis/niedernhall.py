"""
Scraper für die Stadt Niedernhall.
Website: https://www.niedernhall.de/freizeit-kultur/veranstaltungskalender
Pagination: /seite-X/
"""
...
class NiedernhallScraper(BaseScraper):
    """Scraper für Niedernhall Veranstaltungen mit Pagination."""

    SOURCE_NAME = "Stadt Niedernhall"
    BASE_URL = "https://www.niedernhall.de"
    EVENTS_URL = "https://www.niedernhall.de/freizeit-kultur/veranstaltungskalender/seite-1/suche-none"

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74676 Niedernhall"

    # CSS-Selektoren für Niedernhall
    SELECTORS = {
    ...
    def parse_german_date(self, date_str: str) -> Optional[date_class]:
        """
        Parst deutsches Datumsformat: "22.03.2026"
        """
        if not date_str:
            return None
    ...
    def parse_time(self, time_str: str) -> Optional[time_class]:
        """
        Parst Uhrzeitformat: "17:00 Uhr" oder "17:00 Uhr bis 20:00 Uhr" -> nimmt Startzeit
        """
        if not time_str:
            return None
    ...
    def get_all_page_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        Pagination-Struktur bei Niedernhall:
        ...
        # Generiere alle URLs
        base_pattern = "https://www.niedernhall.de/freizeit-kultur/veranstaltungskalender/seite-{}/suche-none"
    ...
    def _parse_single_event(self, container: Tag) -> Optional[ScrapedEvent]:
        ...
        # Koordinaten aus OSM-Link extrahieren (falls vorhanden)
    ...
    def _generate_external_id(
        ...
        # Methode 1: ID aus Container-Attribut (id="hwveranstaltung__record__472")
        container_id = container.get("id", "")
        match = re.search(r"__(\d+)$", container_id)
        if match:
            return f"{match.group(1)}_{event_date}"

        # Methode 2: ID aus URL extrahieren (/veranstaltungskalender/472/...)
        if url:
        ...
        return f"niedernhall_{hash_id}_{event_date}"
