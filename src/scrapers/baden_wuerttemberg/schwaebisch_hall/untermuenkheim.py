"""
Scraper für die Gemeinde Untermünkheim.
API: https://api.cross-7.de/public/calendar/1203/events

Besonderheit: Eigene JSON-API (Cross-7 CMS) - gleiche Schnittstelle wie Öhringen/Langenburg.
...
class UntermuenkheimScraper(BaseScraper):
    """Scraper für Untermünkheim Veranstaltungen (Cross-7 Calendar API)."""

    SOURCE_NAME = "Gemeinde Untermünkheim"
    BASE_URL = "https://www.untermuenkheim.de"
    EVENTS_URL = "https://www.untermuenkheim.de/kultur-tourismus/veranstaltungen-vereine/veranstaltungskalender"

    # API-Endpunkt
    API_URL = "https://api.cross-7.de/public/calendar/1203/events"
    PAGE_SIZE = 150

    # Kategorie-IDs die ausgeschlossen werden sollen
    EXCLUDE_CATEGORY_IDS = [130585, 130584, 130586]

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74547 Untermünkheim"
    ...
    def _parse_api_event(self, item: Dict[str, Any]) -> Optional[ScrapedEvent]:
        ...
        url = f"{self.EVENTS_URL}?c7-item={target_id}" if target_id else self.EVENTS_URL
        ...
        if target_id:
            external_id = f"untermuenkheim_{target_id}_{event_date}"
        else:
            import hashlib
            hash_input = f"{name}_{event_date}".encode("utf-8")
            hash_id = hashlib.md5(hash_input).hexdigest()[:8]
            external_id = f"untermuenkheim_{hash_id}_{event_date}"
