"""
Scraper für die Gemeinde Kupferzell.
Website: https://www.kupferzell.de/freizeit-erlebnis/veranstaltungskalender
Pagination: /seite-X/
"""
...
class KupferzellScraper(BaseScraper):
    """Scraper für Kupferzell Veranstaltungen mit Pagination."""

    SOURCE_NAME = "Gemeinde Kupferzell"
    BASE_URL = "https://www.kupferzell.de"
    EVENTS_URL = "https://www.kupferzell.de/freizeit-erlebnis/veranstaltungskalender/seite-1/suche-none"

    # Für Google Geocoding API - grenzt Suchergebnisse ein
    GEOCODE_REGION = "74635 Kupferzell"

    # CSS-Selektoren für Kupferzell
    SELECTORS = {
    ...
    def parse_german_date(self, date_str: str) -> Optional[date_class]:
        """
        Parst deutsches Datumsformat: "07.02.2026"
        """
        if not date_str:
            return None
    ...
    def get_all_page_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        Pagination-Struktur bei Kupferzell:
        - "Letzte Seite" Link mit title="Letzte Seite" (enthält max Seitenzahl)
        ...
        # Generiere alle URLs
        base_pattern = "https://www.kupferzell.de/freizeit-erlebnis/veranstaltungskalender/seite-{}/suche-none"
    ...
    def _generate_external_id(
        ...
        # Methode 1: ID aus Container-Attribut (id="hwveranstaltung__record__282")
        container_id = container.get("id", "")
        match = re.search(r"__(\d+)$", container_id)
        if match:
            return f"{match.group(1)}_{event_date}"

        # Methode 2: ID aus URL extrahieren (/veranstaltungskalender/282/...)
        if url:
        ...
        return f"kupferzell_{hash_id}_{event_date}"
